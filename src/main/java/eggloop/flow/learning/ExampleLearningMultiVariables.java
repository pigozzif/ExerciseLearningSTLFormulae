...
public class ExampleLearningMultiVariables {
    private static final Logger LOGGER = eggloop.flow.learning.LoggerFactory.get();
...
//    private static double[][][] ds2SpatialValues = Utils.readMatrixMultiFromFile(ds2Times.length, FILE_PATH.apply("temporal/synthData_12.txt"));
    private static eggloop.flow.learning.Learning learning;

    public static void main(String[] args) {
        LOGGER.setLevel(Level.FINE);
        learning = new eggloop.flow.learning.Learning(LOGGER, DISCRIMINATION_FUNCTION);
//        learn(new Random(), LOGGER);
    ...
    private static double[][] core(Random random, DataSetSplit data, Logger logger) {
        ...
        eggloop.flow.learning.GeneticPopulation generation = null;
        int NG = 3;
        ...
        double[] bestFormulaParameters = eggloop.flow.learning.ComputeAverage.averageMultiTrajectory(200, DISCRIMINATION_FUNCTION, variables, ds2Times, positiveTrainingSet, negativeTrainingSet, bestFormula, pop, new double[]{GeneticOptions.min_time_bound, GeneticOptions.max_time_bound}, atTime);
