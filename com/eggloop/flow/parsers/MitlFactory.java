...
import com.eggloop.flow.expr.*;
import com.eggloop.flow.mitl.*;
import com.eggloop.flow.modelChecking.SignalFunction;
import com.eggloop.flow.modelChecking.SignalFunctionType;
...
public class MitlFactory {
	...
	public MitlPropertiesList constructProperties(String text) {
		...
		com.eggloop.flow.parsers.MiTLParser parser = new com.eggloop.flow.parsers.MiTLParser(tokens);
		com.eggloop.flow.parsers.MiTLParser.eval_return r = null;
		try {
			r = parser.eval();
		...
		if (ast.getType() == 0)
			constructModel(ast);
		else if (ast.getType() == com.eggloop.flow.parsers.MiTLParser.CONST)
	...
	public void constructDeclaration(String text) throws RecognitionException {
		...
		CommonTokenStream tokens = new CommonTokenStream(lex);
		com.eggloop.flow.parsers.MiTLParser parser = new com.eggloop.flow.parsers.MiTLParser(tokens);
		com.eggloop.flow.parsers.MiTLParser.declaration_return r = parser.declaration();
		Tree ast = (Tree) r.getTree();
		constructDeclaration(ast);
	}
	...
	public boolean constructProperty(String text) throws RecognitionException {
		...
		CommonTokenStream tokens = new CommonTokenStream(lex);
		com.eggloop.flow.parsers.MiTLParser parser = new com.eggloop.flow.parsers.MiTLParser(tokens);
		com.eggloop.flow.parsers.MiTLParser.exprOR_return r = parser.exprOR();
		Tree ast = (Tree) r.getTree();
		constructMiTLStatement(ast);
	...
	private void constructModel(Tree astRoot) {
		for (int i = 0; i < astRoot.getChildCount(); i++) {
			...
			if (type == com.eggloop.flow.parsers.MiTLParser.CONST)
	...
	private void constructDeclaration(Tree node) {
		...
		switch (type) {
		case com.eggloop.flow.parsers.MiTLParser.DOUBLE:
			propertiesList.addConstant(name, "double");
			break;
		case com.eggloop.flow.parsers.MiTLParser.INT:
			propertiesList.addConstant(name, "int");
			break;
		case com.eggloop.flow.parsers.MiTLParser.BOOL:
	...
	private ArithmeticExpression constructExpression(Tree node) {
		...
		final int type = node.getType();
		switch (type) {
		case com.eggloop.flow.parsers.MiTLParser.INTEGER:
		case com.eggloop.flow.parsers.MiTLParser.FLOAT:
			return new ArithmeticConstant(Double.parseDouble(node.getText()));

		case com.eggloop.flow.parsers.MiTLParser.ID:
		...
		case com.eggloop.flow.parsers.MiTLParser.PLUS:
		case com.eggloop.flow.parsers.MiTLParser.MULT:
		case com.eggloop.flow.parsers.MiTLParser.DIV:
			return constructBinExpression(node);

		case com.eggloop.flow.parsers.MiTLParser.MINUS:
	...
	private MiTL constructMiTL(Tree node) {
		final int type = node.getType();
		switch (type) {
		case com.eggloop.flow.parsers.MiTLParser.U:
			return constructUntil(node);
		case com.eggloop.flow.parsers.MiTLParser.F:
			return constructFinally(node);
		case com.eggloop.flow.parsers.MiTLParser.G:
			return constructGlobally(node);

		case com.eggloop.flow.parsers.MiTLParser.AND:
			return new MitlConjunction(constructMiTL(node.getChild(0)),
					constructMiTL(node.getChild(1)));
		case com.eggloop.flow.parsers.MiTLParser.OR:
			return new MitlDisjunction(constructMiTL(node.getChild(0)),
					constructMiTL(node.getChild(1)));
		case com.eggloop.flow.parsers.MiTLParser.NOT:
			return new MitlNegation(constructMiTL(node.getChild(0)));

		case com.eggloop.flow.parsers.MiTLParser.TRUE:
			return new MitlTrue();
		case com.eggloop.flow.parsers.MiTLParser.FALSE:
	...
	private MitlUntil constructUntil(Tree node) {
		...
		if (timebound.getType() == com.eggloop.flow.parsers.MiTLParser.COMMA) {
